...
select last_name, count(*) as `number of occurrences` 
...
select last_name, count(*) as `number of occurrences`
	from actor group by last_name having `number of occurrences`>1;
...
select staff.first_name, staff.last_name, sum(payment.amount) from staff
    ...
    where payment.payment_date REGEXP '^2005-08'
    group by staff.staff_id;


-- 6c. List each film and the number of actors who are listed for that film.
-- Use tables film_actor and film. Use inner join
select film.title, count(1) from film
	join film_actor
...
select film.title, count(1) as num_times_rented from film
...
select category.name, sum(payment.amount) as total_revenue from category
...
create view top_5_genres as select category.name, sum(payment.amount) as total_revenue from category
	join film_category on category.category_id = film_category.category_id
    join inventory on inventory.film_id = film_category.film_id
    join rental on inventory.inventory_id = rental.inventory_id
    join payment on rental.rental_id = payment.rental_id
